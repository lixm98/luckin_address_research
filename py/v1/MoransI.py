...
# 1. Load residuals
df = pd.read_csv("csv/residuals.csv")
...
plt.tight_layout()
plt.savefig("png/moran_scatter.png", dpi=150, bbox_inches='tight')
plt.close()
print("\n✓ Moran's I scatter plot saved to png/moran_scatter.png")
...
ax.set_aspect('equal')
plt.tight_layout()
plt.savefig("png/lisa_map.png", dpi=150, bbox_inches='tight')
plt.close()
print(f"✓ LISA map saved to png/lisa_map.png")


# Save LISA results for subsequent GWLR use
df[['lon_wgs84', 'lat_wgs84', 'case', 'pred_proba', 'pearson_residual', 'lisa_label']].to_csv(
    "csv/moran_lisa_results.csv", index=False, encoding='utf-8-sig'
)
print("✓ LISA results saved to csv/moran_lisa_results.csv")