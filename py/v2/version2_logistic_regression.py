...
print("\nRecommendation: VIF < 5 OK; 5-10 Caution; >10 Severe collinearity needs to be addressed")
...
print("OR > 1: Higher values of this feature increase the likelihood of being a Luckin store")
print("OR < 1: Higher values of this feature decrease the likelihood of being a Luckin store")
print("Count type (OR directly corresponds to \"one more unit\"); Distance type (OR corresponds to \"one standard deviation further\")")
...
ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig("png/roc_curve_excel50.png", dpi=150, bbox_inches='tight')
plt.close()
print(f"\n✓ ROC Curve saved to png/roc_curve_excel50.png")
...
residuals.to_csv("csv/residuals_excel50.csv", index=False, encoding='utf-8-sig')
print(f"✓ Residuals saved to csv/residuals_excel50.csv ({len(residuals)} rows)")

# Save the coefficient table for dissertation use
odds_table.to_csv("csv/logit_coefficients_excel50.csv", encoding='utf-8-sig')
print(f"✓ Coefficient table saved to csv/logit_coefficients_excel50.csv")