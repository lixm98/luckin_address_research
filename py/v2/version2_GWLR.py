...
for i, f in enumerate(FEATURES):
    ...
    out[f'vif_{f}']  = vifs[:, i]
out.to_csv("csv/gwlr_local_coefficients_excl50.csv", index=False, encoding='utf-8-sig')
print("\n✓ csv/gwlr_local_coefficients_excl50.csv has been saved (with local VIF and condition number)")
...
plt.tight_layout()
plt.savefig("png/gwlr_coefficient_maps_excl50.png", dpi=150, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig("png/roc_logistic_vs_gwlr_excl50.png", dpi=150, bbox_inches='tight')
plt.close()
print("✓ Picture saved")