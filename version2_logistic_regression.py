...
ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig("roc_curve_excel50.png", dpi=150, bbox_inches='tight')
plt.close()
print(f"\n✓ ROC Curve saved to roc_curve_excel50.png")
...
residuals.to_csv("residuals_excel50.csv", index=False, encoding='utf-8-sig')
print(f"✓ Residuals saved to residuals_excel50.csv ({len(residuals)} rows)")

# Save the coefficient table for dissertation use
odds_table.to_csv("logit_coefficients_excel50.csv", encoding='utf-8-sig')
print(f"✓ Coefficient table saved to logit_coefficients_excel50.csv")